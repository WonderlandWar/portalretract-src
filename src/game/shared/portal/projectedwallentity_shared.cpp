...
#ifdef GAME_DLL
		pEntity->Teleport( &vNewPos, NULL, &vNewVel );
#else
		AssertMsg( false, "Teleport needs to be implemented to C_BaseEntity" );
		pEntity->SetAbsOrigin( vNewPos );
		pEntity->SetAbsAngles( vNewAngles );
		pEntity->SetAbsVelocity( vNewVel );
		IPhysicsObject *pObj = pEntity->VPhysicsGetObject();
		if ( pObj )
		{
			pObj->SetPosition( vNewPos, vNewAngles, true );
			pObj->SetVelocity( &vNewVel, NULL );
		}
#endif
		return;
	}
	...
	else if ( pEntity->IsPlayer() && m_bIsHorizontal )
	{
		...
		if ( !pPlayer->m_Local.m_bDucked )
		{
			...
			if ( !stuckTrace.startsolid )
			{
				...
				pPlayer->Teleport( &vNewPos, NULL, &vNewVel );
#else
				AssertMsg( false, "Teleport needs to be implemented to C_BaseEntity" );
				pPlayer->SetNetworkOrigin( vNewPos );
				pPlayer->SetNetworkAngles( vNewAngles );
...
#else
			AssertMsg( false, "Teleport needs to be implemented to C_BaseEntity" );
			pEntity->SetNetworkOrigin( vNewPos );
			pEntity->SetNetworkAngles( vNewAngles );
			pEntity->SetAbsVelocity( vNewVel );
#endif
#if defined( GAME_DLL )
			if ( wall_debug.GetBool() )
