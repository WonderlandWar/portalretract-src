...
CBaseEntity *UTIL_NearestEntityInList( Vector point, CUtlVector<CBaseEntity*> *pEntityList )
{
	CBaseEntity *pNearest = NULL;
	float flNearestDistance = FLT_MAX;

	for ( int i = 0; i < pEntityList->Count(); ++i )
	{
		CBaseEntity *pEntity = pEntityList->Element( i );
		float flDist = (pEntity->WorldSpaceCenter() - point).LengthSqr();
		
		if ( flDist < flNearestDistance )
		{
			pNearest = pEntity;
